...
class EncoderDecoder_forEGAdap(BaseSegmentor):
    ...
    def __init__(self,
                 backbone,
                 decode_head,
                 discriminator_E=None,
                 discriminator_F=None,
                 neck=None,
                 auxiliary_head=None,
                 train_cfg=None,
                 test_cfg=None,
                 pretrained=None,
                 init_cfg=None):
        super(EncoderDecoder_forEGAdap, self).__init__(init_cfg)
        ...
        self.discriminator_E = builder.build_discriminator(discriminator_E)
    ...
    def discriminatorE_forward(self, seg_pred):
        outputs = dict()
        outputs['pred_s_dis'] = self.discriminator_E(seg_pred['pred_s'])
        outputs['pred_t_dis'] = self.discriminator_E(seg_pred['pred_t'])
        return outputs

    ## added by LYU: 2022/05/17
    def discriminatorF_forward(self, feature):
        outputs = dict()
        outputs['f_s_dis'] = self.discriminator_F(feature['f_s'])
        outputs['f_t_dis'] = self.discriminator_F(feature['f_t'])
        return outputs
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        optimizer['discriminator_E'].zero_grad()
        ...
        self.set_requires_grad(self.discriminator_E, False)
        ...
        disE_outputs = self.discriminatorE_forward(seg_output_adv)
        disE_outputs['pred_t_dis'] = resize(
            input=disE_outputs['pred_t_dis'],
            size=data_batch['B_img'].shape[2:],
            mode='bilinear',
            align_corners=self.align_corners)
        disE_outputs['pred_s_dis'] = resize(
            input=disE_outputs['pred_s_dis'],
            size=data_batch['img'].shape[2:],
            mode='bilinear',
            align_corners=self.align_corners)
        loss_adv_E, log_vars_adv_E = self._get_gan_loss(self.discriminator_E, disE_outputs['pred_t_dis'], 'pred_t_disE_seg', 1)
        log_vars.update(log_vars_adv_E)

        seg_feature_adv = dict()
        seg_feature_adv['f_t'] = self.sw_softmax(seg_outputs['f_t'][-1])
        seg_feature_adv['f_s'] = self.sw_softmax(seg_outputs['f_s'][-1])
        disF_outputs = self.discriminatorF_forward(seg_feature_adv)
        disF_outputs['f_t_dis'] = resize(
            input=disF_outputs['f_t_dis'],
            size=data_batch['B_img'].shape[2:],
            mode='bilinear',
            align_corners=self.align_corners)
        disF_outputs['f_s_dis'] = resize(
            input=disF_outputs['f_s_dis'],
            size=data_batch['img'].shape[2:],
            mode='bilinear',
            align_corners=self.align_corners)
        loss_adv_F, log_vars_adv_F = self._get_gan_loss(self.discriminator_F, disF_outputs['f_t_dis'], 'f_t_disF_seg', 1)
        loss_adv = loss_adv_E + loss_adv_F
        loss_stage1 = loss_seg + loss_adv 
        loss_stage1.backward()
        log_vars.update(log_vars_adv_F)
        ...
        # 3. train discriminator_E with source
        self.set_requires_grad(self.discriminator_E, True)
        ...
        disE_outputs = self.discriminatorE_forward(seg_outputs_detach)
        disE_outputs['pred_t_dis'] = resize(
            input=disE_outputs['pred_t_dis'],
            size=data_batch['B_img'].shape[2:],
            mode='bilinear',
        ...
        disE_outputs['pred_s_dis'] = resize(
            ...
            mode='bilinear',
            align_corners=self.align_corners)
        loss_adv_dsE, log_vars_adv_dsE = self._get_gan_loss(self.discriminator_E, disE_outputs['pred_s_dis'], 'pred_s_disE_d', 1)
        loss_adv_dsE.backward()
        log_vars.update(log_vars_adv_dsE)
        loss_adv_dtE, log_vars_adv_dtE = self._get_gan_loss(self.discriminator_E, disE_outputs['pred_t_dis'], 'pred_t_disE_d', 0)
        loss_adv_dtE.backward()
        log_vars.update(log_vars_adv_dtE)
        optimizer['discriminator_E'].step()

        # 3. train discriminator_E with source
        self.set_requires_grad(self.discriminator_F, True)
        f_t_dis = seg_outputs['f_t'][-1].detach()
        f_s_dis = seg_outputs['f_s'][-1].detach()
        ## modified by LYU: 2022/04/18
        f_t_dis = self.sw_softmax(f_t_dis)
        f_s_dis = self.sw_softmax(f_s_dis)
        seg_feature_detach = dict()
        seg_feature_detach['f_t'] = f_t_dis
        seg_feature_detach['f_s'] = f_s_dis
        disF_outputs = self.discriminatorF_forward(seg_feature_detach)
        disF_outputs['f_t_dis'] = resize(
            input=disF_outputs['f_t_dis'],
            size=data_batch['B_img'].shape[2:],
            mode='bilinear',
        ...
        disF_outputs['f_s_dis'] = resize(
            ...
            mode='bilinear',
            align_corners=self.align_corners)
        loss_adv_dsF, log_vars_adv_dsF = self._get_gan_loss(self.discriminator_F, disF_outputs['f_s_dis'], 'f_s_disF_d', 1)
        loss_adv_dsF.backward()
        log_vars.update(log_vars_adv_dsF)
        loss_adv_dtF, log_vars_adv_dtF = self._get_gan_loss(self.discriminator_F, disF_outputs['f_t_dis'], 'f_t_disF_d', 0)
        loss_adv_dtF.backward()
        log_vars.update(log_vars_adv_dtF)
